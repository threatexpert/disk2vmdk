...
BOOL CImageMaker::start()
{
    ...
    if (_wcsicmp(m_format.c_str(), L"DD") == 0) {
        ...
        // VMDK / VHD / VDI — use CVDiskWriter
        bufpool_count = 10;

        // VHD needs capacity aligned to 2MB block boundary
        if (_wcsicmp(m_format.c_str(), L"VHD") == 0) {
            const uint64_t VHD_BLOCK_SIZE = 1024 * 1024 * 2;
            if (m_nSourceSize % VHD_BLOCK_SIZE) {
                uint64_t nAligned = VHD_BLOCK_SIZE - (m_nSourceSize % VHD_BLOCK_SIZE);
...
DWORD CImageMaker::ThreadWriter()
{
    ...
    for (;;) {
        ...
        if (buf->zero_length) {
            ...
            if (m_WorkingMode == Mode_VD) {
                // Virtual disk format: notify writer about zero range (sparse, not written)
                if (!m_vd_writer.WriteZero(m_nDestTotalWritten, left)) {
                    m_writer_err = LSTRW(RID_WriteFileError);
                    m_writer_err += m_vd_writer.lasterr();
                    goto _ERROR;
                }
                m_nDestTotalWritten += left;
                if (m_pCB)
                    m_pCB->ImageMaker_OnCopied(m_nDestTotalWritten, m_nVDCapacity, m_nDestDataWritten, m_nSourceDataSpaceSize);
            }
            else {
                // DD mode: must write actual zeros to file
                for (; left > 0;) {
                    if (m_exit_flag) {
                        goto _END;
                    }
            ...
            if (m_WorkingMode == Mode_VD) {
                if (!m_vd_writer.Write(m_nDestTotalWritten, buf->p, buf->data_length)) {
                    m_writer_err = LSTRW(RID_WriteFileError);
                    m_writer_err += m_vd_writer.lasterr();
                    goto _ERROR;
                }
